...
exports.listDir = async function (parent, deep, root) {
  ...
  if (deep > 0) {
    ...
    for (const file of files) {
      ...
      if (info.isDirectory()) {
        item.type = 'directory';
        item.label = file;
        item.children = await exports.listDir(path.join(parent, file), deep - 1, root);
      }
      result.push(item);
    }
...
exports.readFile = async function (file, root) {
