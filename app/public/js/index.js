...
layui.use(['bodyTab', 'form', 'element', 'jquery', 'laytpl','request'], function () {
	var form = layui.form,
	...
	skins();
	// 弹框
	modalShow = function (tit, con, width, shadeClose) {
	...
	if (sessionStorage.user) {
		$('.userName').text(sessionStorage.user && JSON.parse(sessionStorage.user).name);
	} else {
	...
	$("#changePwd").on("click", function () {
		modalShow('修改密码', $('#change_pwd').html().replace(/\{\{[\s\S]*?\}\}/g, ''), '400px');
	})
	// 提交修改密码
	// 表单验证规则
	form.verify({});
	...
	form.on('submit(f-submit)', function (data) {
		data.field.id = JSON.parse(sessionStorage.user).id;
