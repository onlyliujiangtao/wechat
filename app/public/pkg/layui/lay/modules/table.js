...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function (exports) {
    ...
    Class.prototype.render = function () {
        ...
        that.events();
        return thisTable.call(this);
    };

    //根据列类型，定制化参数
    Class.prototype.initOpts = function (item, oriItem) {
        var that = this,
            options = that.config
            , initWidth = {
                checkbox: 48,
        ...
        //让 type 参数兼容旧版本
        if (oriItem.checkbox) item.type = "checkbox";
        if (oriItem.space) item.type = "space";
        if (!oriItem.type) item.type = "normal";
        if (item.type !== "normal") {
            item.unresize = true;
            item.width = oriItem.width || initWidth[item.type];
        } else {
            item.width = oriItem.width
        }
    };
    ...
    Class.prototype.setArea = function () {
        ...
        layui.each(options.cols, function (i1, item1) {
            var oTemp = oldCols[i1];
            layui.each(item1, function (i2, item2) {
                var oriItem = oTemp[i2];
                var width;
        ...
        layui.each(options.cols, function (i1, item1) {
            ...
            layui.each(item1, function (i2, item2) {
                temp += item2.width;
                if (len - 1 === i2 ) {
                    var num = cntrWidth - temp;
                    num > 0 && (item2.width += num);
                }
            });
        });
        //高度铺满：full-差距值
                ...
                that.eachCols(function (i3, item3) {
                    ...
                    var td = ['<td data-field="' + field + '" ' + function () {
                        ...
                        , '<div class="layui-table-cell laytable-cell-' + function () { //返回对应的CSS类标识
                            ...
                            //渲染复选框列视图
                            if (item3.type === 'checkbox' || item3.type === "radio") {
                                var disSelect, setSelect;
                                if (item3.disSelect && typeof(item3.disSelect) === 'function') {
                                    disSelect = item3.disSelect(item1);
                                }
                                if (item3.setSelect && typeof(item3.setSelect) === 'function') {
                                    setSelect = item3.setSelect(item1);
                                }
                                return '<input data-type="' + item3.type + '" type="checkbox" ' + (disSelect ? 'disabled' : '') + ' name="layTableCheckbox" lay-skin="primary" ' + function () {
                                        var checkName = table.config.checkName;
                                        if (setSelect) {
                                            item3[checkName] = true;
                                        }
                                        //如果是全选
                                        if (item3[checkName]) {
    ...
    //单选
    Class.prototype.setRadioData = function (index, checked) {
        var $checkElem = this.layBody.find('input[name="layTableCheckbox"]');
        var that = this
            , options = that.config
        ...
        $.each(thisData, function (i, item) {
            item[options.checkName] = false;
            $checkElem.eq(i).prop('checked', false);
        });
        thisData[index][options.checkName] = checked;
        $checkElem.eq(index).prop('checked', checked);
    ...
    Class.prototype.syncCheckAll = function () {
        ...
        var isEnableAll = true;
        var $checkElem = that.layBody.find('input[name="layTableCheckbox"]');
        var len = $checkElem.length;
        var iDisnum = 0;
        $.each($checkElem, function (i, item) {
            if (!item.disabled) {
        ...
        if (table.checkStatus(that.key).isAll || (iDisnum !== len && isEnableAll)) {
    ...
    Class.prototype.events = function () {
        ...
        that.elem.on('click', 'input[name="layTableCheckbox"]+', function () {
            ...
            if (type === 'checkbox' || isAll) {
                ...
                that.setRadioData(index, checked);
            }
            layui.event.call(this, MOD_NAME, 'checkbox(' + filter + ')', {
                checked: checked
                , data: table.cache[that.key] ? (table.cache[that.key][index] || {}) : {}
                , type: table.checkStatus(that.key).isAll ? 'all' : 'one'
            });
        });

        // 行点击事件
        that.layBody.on('click', 'tr', function (e) {
            var $checkbox = $(e.target).closest('.layui-form-checkbox');
            var index = $(this).data('index');
            var len = $checkbox.length;
            $checkbox = !len ? $(this).find('.layui-form-checkbox').prev() : $checkbox.prev();
            if (!len && that.config.isRowCheck) {
            ...
            layui.event.call(this, MOD_NAME, 'rowClick(' + filter + ')', {
                checked: $checkbox.length ? $checkbox[0].checked : '',
                data: table.cache[that.key] ? (table.cache[that.key][index] || {}) : {},
                type: table.checkStatus(that.key).isAll ? 'all' : 'one'
            });
        });
    ...
    // 重新设置宽度
    Class.prototype.reWidth = function () {
        var style = [
            '<style>'
            , '{{# layui.each(d.data.cols, function(i1, item1){'
            , 'layui.each(item1, function(i2, item2){ }}'
        ...
        this.setArea();
        var res = laytpl(style.join('')).render({
            data: this.config
            , index: this.index //索引
        });
        this.elem.find('style').remove().end().append($(res));
    };
